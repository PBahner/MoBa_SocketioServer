...
switches = [Switches(0, 0, PcfPinRef(0, 2, 0x39, 0), PcfPinRef(0, 2, 0x39, 1)),
            Switches(1, 0, PcfPinRef(0, 2, 0x38, 7), PcfPinRef(0, 2, 0x38, 6)),
            Switches(2, 0, PcfPinRef(0, 2, 0x38, 4), PcfPinRef(0, 2, 0x38, 5)),
            Switches(3, 1),
            Switches(4, 1),
            Switches(5, 0, PcfPinRef(0, 2, 0x38, 2), PcfPinRef(0, 2, 0x38, 3)),
            Switches(6, 0, PcfPinRef(0, 2, 0x38, 1), PcfPinRef(0, 2, 0x38, 0))]
...
class CanCommunicator:
    ...
    def send_msg(id, data):
        ...
        with can.interface.Bus(channel=CanCommunicator.channel,
            ...
            try:
                bus.send(msg)
                print(f"[CAN] Message sent on {bus.channel_info} id: {msg.arbitration_id} msg: {msg.data}")
                return True
            except can.CanError:
...
class Esp32Communicator(socketio.Namespace):
    ...
    @staticmethod
    def send_switch_states():
        CanCommunicator.request_inputs(0, 2, 0x38)
        CanCommunicator.request_inputs(0, 2, 0x39)
        data = {switch.id: switch.current_pos for switch in switches}
